...
OpenCopilot's Open Source LLM support is based on `llama.cpp` therefore it requires following libraries to work:
...
```bash
pip install "llama-cpp-python[server]" pydantic_settings sse_starlette
```
...
```bash
CMAKE_ARGS="-DLLAMA_METAL=on" pip install "llama-cpp-python[server]" pydantic_settings sse_starlette
```
...
Notice that we are also using a local embeddings model.
