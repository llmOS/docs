...
```
This is most useful for quick validation, for better data quality look into custom scraping solutions or custom document loaders below.

The URL loader supports any file type mentioned above and falls back to html (without Javascript) if it cannot detect the file type.
To load a file other than HTML the url needs to point directly to the file.
