...
To use the knowledge base, you need to set up a Weaviate vector store (TODO this needs to be made much easier):

```shell
cd opencopilot && docker-compose -f docker-compose-weaviate.yml up -d
```
...
To ingest new data, the copilot needs to be restarted.