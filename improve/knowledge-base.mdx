...
You can add URLs that point to any supported file type (see above). If file type cannot be detected, the URL is parsed as HTML (without any Javascript execution). To load a file other than HTML the url needs to point directly to the file.
